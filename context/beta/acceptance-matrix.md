# Beta Acceptance Matrix — `0.1.0 (6)`
...
| Swift formatting/lint/unit/UI suite | PASS | Build-6 `make ci` on 2026-08-10: 0 formatting/lint violations across 85 Swift files; 96/96 unit tests and 19/19 UI tests pass. |
...
| Archive configuration / secret scan | PASS | Signed build `0.1.0 (6)` archive validates with deployment target 18.0, expected bundle ID and team, app/dependency privacy manifests, and no privileged-key or development-endpoint bundle scan hits. |
...
|---|---|---|
| Hosted migrations 02–10 | PASS | Pre-change logical backup captured; Phase 01 ledger mismatch reconciled after exact schema comparison; all 8 Git timestamps remained present and project health was `ACTIVE_HEALTHY` on 2026-08-10. |
...
| Signed internal baseline `0.1.0 (5)` | PASS | Xcode archive validation and the six-category/no-tracking privacy report passed; App Store Connect processed build 5; Ed installed the Apple-hosted internal build on the physical iPhone and confirmed existing Home, Log/photos, Profile, Tackle Box, and Map data. |
| Signed internal RC `0.1.0 (6)` | NOT RUN | Signed archive validation and upload passed; App Store Connect processed build 6 and assigned it to `Reel Records Internal`. Install it over build 5 and smoke-test existing data plus Issues #1–3. |
...
| External App Review and invite-only group | BLOCKED | Build 5 is `Testing` in the email-only `Reel Records Friends & Family` group as of 2026-08-10. Build 6 remains intentionally unassigned until its internal install/smoke gate passes; then it must be submitted for external review. |
