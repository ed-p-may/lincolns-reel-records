...
- [ ] Confirm `MARKETING_VERSION = 0.1.0` and `CURRENT_PROJECT_VERSION = 4` in `project.yml`.
- [ ] Regenerate the project and run `make ci` from a clean checkout.
- [ ] Confirm the privacy manifest is in the built app root and generate the archive privacy report.
...
- [ ] Verify the CLI/dashboard target is project ref `ptoqkqisgyzypfpjvmvx`; stop if it differs.
- [ ] Capture the hosted migration ledger and a pre-change backup/recovery point.
- [ ] Stop if hosted Phase 01 is recorded as `20260719191234` while Git records
...
- [ ] Review the local migration list in timestamp order; deploy migrations 02–10 once.
- [ ] Deploy the authenticated `delete-account` Edge Function with its server-only environment.
- [ ] Compare the hosted ledger to Git; run two-user table and Storage isolation probes.
- [ ] Test password reset with a receivable beta account; do not use the non-receivable `.test` reviewer
  address for mail delivery evidence.
- [ ] Exercise account deletion only with a disposable beta account; confirm Auth, rows, and objects are gone.
...
- [ ] Upload `0.1.0 (4)` to App Store Connect; record processing and compliance results.
- [ ] Add it to `Reel Records Internal` only; install over `0.1.0 (3)` and as a clean install.
