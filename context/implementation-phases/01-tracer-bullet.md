...
- TestFlight build: `0.1.0 (3)` is signed and uploaded. It is available to the internal group
  `Reel Records Internal`; `ed.p.may@gmail.com` installed the beta on the named test phone. This was the
  Phase 01 release evidence; Phase 11 later replaced its external-review submission with verified build
  `0.1.0 (5)`. Exact current operational identifiers and account roles are recorded in
